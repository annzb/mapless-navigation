...
class SoftMatchingLossScaled(nn.Module):
    ...
    def forward(self, predicted_points, ground_truth_points):
        ...
        if valid_matches.sum() == 0:
            return predicted_points.new_tensor(self.big_dist, requires_grad=True)
        pairwise_distances = pairwise_distances.masked_fill(~valid_matches, self.big_dist)  # Avoid in-place operation
...
class SoftMatchingLoss(nn.Module):
    ...
    def forward(self, predicted_points, ground_truth_points):
        ...
        print('probability_loss', probability_loss)
        if total_loss.isnan():
            raise RuntimeError('Loss is NaN.')
        return total_loss
